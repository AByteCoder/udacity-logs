...
def find_error(cursor):
    ...
    view_failure = "CREATE OR REPLACE VIEW LOG_FAILED AS SELECT DATE(time) "\
        + "AS day, COUNT(*) AS num_failed FROM log "\
        + "WHERE status = '404 NOT FOUND' GROUP BY DATE(time)"
    sql_query = "SELECT DATE(time) AS day,num_failed::float/num_total * 100 "\
        + "AS failure_percent FROM log,LOG_SUCCESS,LOG_FAILED"\
        + " WHERE DATE(time) = LOG_SUCCESS.day AND DATE(time) = "\
        + "LOG_FAILED.day AND (num_failed::float/num_total * 100) >= 1"\
...
def main():
    ...
    question_3 = "3. On which days did more than 1% of requests lead to "\
        + "errors?"
    print("")
    print(question_3)
    print("")
    ...
    print("===================  =========")
    for error in errors:
        print("% - 20s % - 3.1f%%" % (error[0].strftime('%B %-d, %Y'),
                                      error[1]))
    connection.close()
